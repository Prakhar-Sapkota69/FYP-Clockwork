...
class UIManager:
    ...
    def display_games_in_grid(self, games=None):
        ...
        try:
            ...
            # If no games provided, use all games from main window
            if games is None:
                games = self.main_window.games
            ...
            for game in games:
                # Create game card
                card = GameCard(game, self.main_window)
                if card:
            ...
            # Force layout update
            self.games_container.updateGeometry()
            self.games_layout.update()
